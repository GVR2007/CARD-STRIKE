...
def apply_status_effect(target, effect):
    ...
    for e in target.active_effects:
        if e.type == effect.type:
            e.duration = max(e.duration, effect.duration) # Refresh duration
            return

    target.active_effects.append(effect)
    anim_mgr.add_floating_text(f"+{effect.name}", *cell_center(*target_pos(target, grid_ref_only=False)), (255, 255, 255))

def target_pos(card_obj, grid_ref_only=True):
    # Helper to find card position, this is a bit hacky without grid ref
    # For now we skip the animation text specific pos or assume caller handles it
    return (0,0) 
...
def perform_attack_logic(ac, ar, tc, tr, atk, grid, dist=0):
    ...
    if atk.is_healing:
        ...
        if atk.attack_range > 0:
             ...
             reachable = bfs_reachable((ac, ar), atk.attack_range, grid)
