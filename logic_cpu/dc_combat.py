...
def select_position(attacker_card, attacker_pos, target_pos, grid):
    ...
    # We want MINIMIZE cost.
    # Cost = (Dist to Target * 2) + (Safety Cost * 5)
    tx, ty = target_pos
    for tile in valid_candidates:
        ...
        if path_cost == float('inf'):
            path_cost = 999 # Unreachable by DP (e.g. valid by BFS but blocked by hazard logic?)
            
        dist = abs(cx - tx) + abs(cy - ty)
        
        # Lower is better.
        # WEIGHTS:
        # Distance: 10 (High priority to close gap)
        # Safety/Path Cost: 2 (Low priority for normal tiles, but Fire tiles cost 10 -> 20 penalty)
        # 
        # Example 1 Step Normal: Dist-1, Cost+1 -> Score change: -10 + 2 = -8 (Good, move)
        # Example 1 Step Fire:   Dist-1, Cost+10 -> Score change: -10 + 20 = +10 (Bad, stay)
        scores[tile] = dist * 10 + path_cost * 2
        # print(f"  [MoveEval] {tile}: Dist={dist} Cost={path_cost} -> S={scores[tile]}")
    ...
    if best and scores[best] > 50:
        # If best move is still very dangerous/far, maybe stay put?
        # Check current pos score
        current_score = scores.get(attacker_pos, 9999)
        if current_score < scores[best]:
            return attacker_pos
