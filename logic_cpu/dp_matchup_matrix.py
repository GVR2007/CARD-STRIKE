def optimize_matchups(attackers, targets, score_fn):
    ...
    # DP State: dp[mask] = max score using subset of attackers represented by mask
    # We assign attackers 0..k to targets? No, order matters.
    # Actually, iterate through targets 0..m-1.
    # dp[mask] = max score after assigning to first 'count_set_bits(mask)' targets using specific attackers?
    # No, usually:
    # dp[i][mask] = max score assigning first i attackers to subset of targets defined by mask.
    # Or simpler for N=M:
    # dp[mask]: max score using subset of attackers (mask) assigned to first k targets?
    
    # dp[state] = (score, [(attacker, target), ...])
    memo = {}
