...
class StealingPhase:
    ...
    def cpu_turn(self):
        ...
        # ==================================================
        # STEP 1: DIVIDE — Define independent subproblems
        # ==================================================
        actions = []
        ...
        # 0. BACKTRACKING: Subset Sum for Stat Targets
        # Check if we can form a perfect HP combo (e.g. 200 HP exactly) with current hand + steal candidate
        try:
            from logic_cpu.backtracking_deck_subset import find_subset_sum_exact
            # Target: try to get exactly 250 HP total in deck (just a fun goal)
            current_deck_hp = sum(c['hp'] for c in self.cpu_deck)
            target_hp = 250
            needed = target_hp - current_deck_hp
            
            # Find if any card in player hand helps reach this
            candidates = [c for c in self.player_hand] # indices
            # mapping idx to card data
            candidate_cards = [CARD_POOL[i] for i in candidates]
            
            # This is slightly hacked: find_subset used object attributes, we have dicts.
            # We need to wrap them or adjust find_subset.
            # Let's adjust algorithm usage or wrapper.
            # actually find_subset expects objects with attributes.
            # Let's simple-wrap
            class Wrapper:
                def __init__(self, d, i): self.hp = d['hp']; self.idx = i
            
            wrappers = [Wrapper(CARD_POOL[i], i) for i in candidates]
            matches = find_subset_sum_exact(wrappers, needed, 'hp')
            
            if matches:
                # If we found a subset, prioritize the first card of that subset for stealing
                # This ensures we work towards the goal
                best_subset_card_idx = matches[0][0].idx
                steal_card = best_subset_card_idx
                steal_score = 999 # Priority
                print(f"[Backtracking] Found exact HP subset match! Stealing {CARD_POOL[steal_card]['name']}")
                
        except Exception as e:
            print(f"[Backtracking] Error: {e}")

        if steal_card is None and self.player_hand and len(self.cpu_deck) < 3:
            # Use DAC steal target selection
            steal_card = select_steal_target(self.player_hand, CARD_POOL,self.cpu_hand)
            if steal_card is not None:
                steal_score = self._card_score(steal_card)
        actions.append(("STEAL", steal_score, steal_card))
        ...
        if self.player_hand and len(self.cpu_deck) >= 3:
            ...
            if gain > 0:
                swap_score = gain
                swap_card = best_player
                swap_out = worst_cpu
        actions.append(("SWAP", swap_score, (swap_card, swap_out)))

        # ---------- Subproblem 3: RETAIN ----------
        retain_score = 0
        retain_card = None
        if self.cpu_hand:
            # Use new DC Retain Logic
            try:
                from logic_cpu.dc_retain import select_best_retain
                retain_card_idx = select_best_retain(self.cpu_hand, CARD_POOL)
                if retain_card_idx is not None:
                    retain_card = retain_card_idx
                    retain_score = self._card_score(retain_card)
            except ImportError:
                # Fallback in case of import error
                retain_card = max(self.cpu_hand, key=lambda c: self._card_score(c))
                retain_score = self._card_score(retain_card)
                
        actions.append(("RETAIN", retain_score, retain_card))

        # ==================================================
        # STEP 2: CONQUER — Solve each subproblem independently
        # (already done via scoring)
        # ==================================================

        # ==================================================
        # STEP 3: COMBINE — MAX using Divide & Conquer
        # ==================================================
        def dac_max(arr):
            if len(arr) == 1:
                return arr[0]
