        ...
        for r in range(GRID_ROWS):
            ...
            if selected_pos:
                ...
                if sel_card and sel_card.owner == "player":
                    ...
                    move_reach = bfs_reachable((sc, sr), sel_card.move_range, grid, diagonal=False)
                    ...
                    atk_reach = bfs_reachable((sc, sr), max_range, grid, diagonal=True)
    ...
    for c in range(grid.cols):
        for r in range(grid.rows):
            ...
            if card.burn_duration > 0:
                bs = FONT_MICRO.render(f"BURN {card.burn_duration}", True, E_FIRE_GLOW)
                screen.blit(bs, (hx, status_y))
                status_y -= 10
            
            # 2. Stun/Root
            if card.stun_duration > 0:
                ss = FONT_MICRO.render(f"STUN {card.stun_duration}", True, (200, 200, 200))
                screen.blit(ss, (hx, status_y))
                status_y -= 10
            elif card.root_duration > 0:
                rs = FONT_MICRO.render(f"ROOT {card.root_duration}", True, (150, 200, 100))
